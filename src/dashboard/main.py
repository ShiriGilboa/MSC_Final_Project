...
try:
    ...
    def normalize_text(text: str) -> str:
        ...
        # Remove apostrophes only (Option 1: remove apostrophes instead of splitting with spaces)
        # This matches baseline output like "wasnt" instead of "wasn t"
        text = text.replace("'", "")
...
def calculate_wer(ground_truth: str, hypothesis: str) -> float:
    """Calculate Word Error Rate (WER) between two transcriptions using jiwer library."""
    try:
        from jiwer import wer
        return wer(reference=ground_truth, hypothesis=hypothesis)
    except ImportError:
        # Fallback to manual calculation if jiwer is not available
        return calculate_wer_manual(ground_truth, hypothesis)

def calculate_wer_manual(ground_truth: str, hypothesis: str) -> float:
    """Manual WER calculation as fallback."""
    # Tokenize both transcriptions
    ground_truth_tokens = [t.lower() for t in re.findall(r'\b\w+\b', ground_truth)]
    hypothesis_tokens = [t.lower() for t in re.findall(r'\b\w+\b', hypothesis)]
    
    # Create a matrix of edit distances
    matrix = np.zeros((len(ground_truth_tokens) + 1, len(hypothesis_tokens) + 1))
    
    # Initialize the first row and column of the matrix
    for i in range(len(ground_truth_tokens) + 1):
        matrix[i][0] = i
    for j in range(len(hypothesis_tokens) + 1):
        matrix[0][j] = j
    
    # Fill in the rest of the matrix
    for i in range(1, len(ground_truth_tokens) + 1):
        for j in range(1, len(hypothesis_tokens) + 1):
            if ground_truth_tokens[i-1] == hypothesis_tokens[j-1]:
                matrix[i][j] = matrix[i-1][j-1]
            else:
                matrix[i][j] = min(matrix[i-1][j] + 1, matrix[i][j-1] + 1, matrix[i-1][j-1] + 1)
    
    # The edit distance is the value in the bottom-right corner of the matrix
    return matrix[-1][-1] / len(ground_truth_tokens) if len(ground_truth_tokens) > 0 else 0
...
def show_sample_analysis(df):
    ...
    if sample_idx is not None:
        ...
        with col3:
            ...
            # Recalculate baseline WER using new normalization
            if 'transcript' in sample and 'stt_raw_norm' in sample and pd.notna(sample['stt_raw_norm']):
                ground_truth = sample['transcript']
                baseline_text = sample['stt_raw_norm']
                normalized_ground_truth = normalize_text(ground_truth)
                normalized_baseline = normalize_text(baseline_text)
                baseline_wer = calculate_wer(normalized_ground_truth, normalized_baseline)
                st.metric("Baseline WER (New Norm)", f"{baseline_wer:.4f}")
            else:
                st.metric("Baseline WER", "N/A")
        ...
        for pipeline in pipelines:
            ...
            with st.expander(f"🔧 {friendly_name}", expanded=True):
                ...
                with col2:
                    # Performance metrics - Recalculate WER using new normalization
                    pipeline_norm_col = f'{pipeline}_norm'
                    if pipeline_norm_col in sample and pd.notna(sample[pipeline_norm_col]) and 'transcript' in sample:
                        # Get the pipeline transcription and ground truth
                        pipeline_text = sample[pipeline_norm_col]
                        ground_truth = sample['transcript']
                        
                        # Normalize both using the new normalization method
                        normalized_ground_truth = normalize_text(ground_truth)
                        normalized_pipeline = normalize_text(pipeline_text)
                        
                        # Calculate WER with new normalization
                        pipeline_wer = calculate_wer(normalized_ground_truth, normalized_pipeline)
                        
                        # Compare with baseline WER (also recalculated with new normalization)
                        if 'stt_raw_norm' in sample and pd.notna(sample['stt_raw_norm']):
                            baseline_text = sample['stt_raw_norm']
                            normalized_baseline = normalize_text(baseline_text)
                            baseline_wer = calculate_wer(normalized_ground_truth, normalized_baseline)
                            improvement = baseline_wer - pipeline_wer
                            
                            st.metric("WER (New Norm)", f"{pipeline_wer:.4f}")
                            ...
                            st.metric("WER (New Norm)", f"{pipeline_wer:.4f}")
                            st.info("Baseline WER not available for comparison")
                    else:
                        st.write("*No transcription available for WER calculation*")
                
                with col1:
                    # Pipeline result text (already normalized in CSV)
                    pipeline_norm_col = f'{pipeline}_norm'
                    if pipeline_norm_col in sample and pd.notna(sample[pipeline_norm_col]):
                        pipeline_text = normalize_text(sample[pipeline_norm_col])
